...
long cost_func(int x1, int y1, int x2, int y2, int size){
    ...
    int i1 = x2;
    int j1 = y2;
   v4i32 red_prev = {0, 0, 0, 0};
   v4i32 green_prev = {0, 0, 0, 0};
   v4i32 blue_prev = {0, 0, 0, 0};
   v4i32 red_cur = {0, 0, 0, 0};
   v4i32 green_cur = {0, 0, 0, 0};
   v4i32 blue_cur = {0, 0, 0, 0};
   v4i32 red_error;
   v4i32 green_error;
   v4i32 blue_error;
   v4i32 acc = {0, 0, 0 ,0};
    ...
    for(int i = x1, i1 = x2; i < x1 + size; i++, i1++){
       for(int j = y1, j1 = y2, k = 0; j < y1 + size; j++, j1++){
           ...
           red_cur[(j-y1)%4] += int(colour_cur.red);
           green_cur[(j-y1)%4] += int(colour_cur.green);
           blue_cur[(j-y1)%4] += int(colour_cur.blue);
   }
    }
