...
typedef struct FlashPartInfo {
     ...
     * Used for RDID (0x9f) command
     */
    uint8_t jedec_id[SPI_NOR_MAX_ID_LEN];
    uint8_t jedec_id_len;

    /* ID for READ_ID (0x90) command */
...
#define INFO(_part_name, _jedec_id, _ext_id, _sector_size, _n_sectors, _flags)\
    ...
    .jedec_id_len = (!(_jedec_id) ? 0 : (3 + ((_ext_id) ? 2 : 0))),\
...
#define INFO_EXTENDED(_part_name, _jedec_id, _ext_jedec, _id, \
    ...
    .jedec_id_len = (!(_jedec_id) ? 0 : (3 + ((_ext_jedec) ? 2 : 0))),\
    .id = {(_id >> 8) & 0xff, _id & 0xff}, \
    .id_len = 2, \
    .sector_size = (_sector_size),\
    .n_sectors = (_n_sectors),\
    .page_size = 256,\
...
static const FlashPartInfo known_devices[] = {
    ...
    { INFO("s25fl127s",   0x012018, 0x4d00, 256 << 10,  64, 0) },
    { INFO_EXTENDED("s25fl128p", 0x012018, 0x0300, 0x0117, 256 << 10,  64, 0) },
    ...
    { INFO("w25q128fv",   0xef4018,      0,  64 << 10, 256, ER_4K) },
...
static void decode_new_cmd(Flash *s, uint32_t value)
{
    ...
    case JEDEC_READ:
        ...
        for (i = 0; i < s->pi->jedec_id_len; i++) {
            s->data[i] = s->pi->jedec_id[i];
        }

        s->len = s->pi->jedec_id_len;
        s->pos = 0;
        s->state = STATE_READING_DATA;
        break;
...
void m25p80_init_mr(SSISlave *ss, MemoryRegion *mr)
{
    Flash *s = M25P80(ss);
    memory_region_init_ram_ptr(mr, OBJECT(s), s->pi->part_name, s->size,
                               s->storage);
}
