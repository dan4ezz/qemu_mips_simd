...
static void dp83865_reset(struct MiiTransceiver *s)
{
    uint16_t a[] = {
    0x1140, 0x796d, 0x2000, 0x5c7a, 0x01e1, 0x45e1, 0x0005, 0x0001,
    0x0000, 0x0300, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x3000,
    0xc441, 0xc436, 0x8200, 0x0001, 0x0000, 0x0000, 0x0000, 0x0000,
    0x0044, 0x0000, 0x8000, 0x0000, 0x0000, 0x0000, 0x0036, 0x0001
    };
    int i;
...
static void lxt971_set_link_status(struct MiiTransceiver *mii, int link_down)
{
    if (link_down) {
        mii->mii_data[PHY_STATUS] &= ~MII_SR_LINK_STATUS;
    } else {
        mii->mii_data[PHY_STATUS] |= MII_SR_LINK_STATUS;
    }
}
...
void lxt971_init(struct MiiTransceiver *s, int phy_addr)
{
    ...
    s->set_link_status = lxt971_set_link_status;
...
void dp83865_init(struct MiiTransceiver *s, int phy_addr)
{
    s->reset = dp83865_reset;
    s->readreg = lxt971_readreg;
    s->writereg = lxt971_writereg;
    s->set_link_status = lxt971_set_link_status;
