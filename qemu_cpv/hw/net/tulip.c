...
static void process_tx(TulipState *s)
{
    ...
    while (1) {
        ...
        if (length & TULIP_TDES1_TER) {
            cur_tx_desc = mac_readreg(s, CSR4);
                 ...
                 unsigned size)
{
    ...
    case TULIP_CSR3: /* Start of Receive List */
        ...
        /* Start of Receive List was probably changed, so flush rx queue here */
    ...
    case TULIP_CSR6: /* Command/Mode Register */
        ...
        /* RX could be turned on above, so flush rx queue at first */
        qemu_flush_queued_packets(qemu_get_queue(s->nic));
        break;
...
void tulip_init(DeviceState *dev, TulipState *s, NetClientInfo *info)
{
    int i;
    uint8_t *macaddr;
