...
static uint64_t srisa_ehci_read(void *opaque, hwaddr addr, unsigned size)
{
    switch (addr & 0xF) {
    case SRISA_REG_RID:
        return 0x3380191e;
...
static void srisa_ehci_write(void *opaque, hwaddr addr, uint64_t val,
                                unsigned size)
{
}

static const MemoryRegionOps srisa_ehci_mmio_ops = {
    .read = srisa_ehci_read,
    .write = srisa_ehci_write,
    .valid.min_access_size = 4,
    .valid.max_access_size = 4,
    .endianness = DEVICE_LITTLE_ENDIAN,
};

static void srisa_ehci_init(Object *obj)
{
    EHCISysBusState *i = SYS_BUS_EHCI(obj);
    SRISAEHCIState *f = SRISA_EHCI(obj);
    EHCIState *s = &i->ehci;

    memory_region_init_io(&f->mem_vendor, OBJECT(f), &srisa_ehci_mmio_ops, s,
                          "srisa", 0x10);
    memory_region_add_subregion(&s->mem,
                                0xF0,
                                &f->mem_vendor);
...
typedef struct SRIRSAEHCIClass {
    /*< private >*/
    SysBusEHCIClass parent_class;
    /*< public >*/

    DeviceRealize parent_realize;
} SRISAEHCIClass;

#define SRISA_EHCI_CLASS(class) \
    OBJECT_CLASS_CHECK(SRISAEHCIClass, (class), TYPE_SRISA_EHCI)

#define SRISA_EHCI_DEVICE_GET_CLASS(obj) \
    OBJECT_GET_CLASS(SRISAEHCIClass, (obj), TYPE_SRISA_EHCI)

static void srisa_ehci_realize(DeviceState *dev, Error **errp)
{
    EHCISysBusState *i = SYS_BUS_EHCI(dev);
    SRISAEHCIState *f = SRISA_EHCI(i);
    SRISAEHCIClass *c = SRISA_EHCI_DEVICE_GET_CLASS(dev);

    i->ehci.as = f->dma_as;

    c->parent_realize(dev, errp);
}

static void srisa_ehci_class_init(ObjectClass *oc, void *data)
{
    SysBusEHCIClass *sec = SYS_BUS_EHCI_CLASS(oc);
    DeviceClass *dc = DEVICE_CLASS(oc);
    SRISAEHCIClass *c = SRISA_EHCI_CLASS(oc);
    ...
    dc->realize = srisa_ehci_realize;
}

static const TypeInfo ehci_srisa_sc64_type_info = {
    .name          = TYPE_SRISA_EHCI,
    .parent        = TYPE_SYS_BUS_EHCI,
    .instance_size = sizeof(SRISAEHCIState),
    .instance_init = srisa_ehci_init,
    .class_init    = srisa_ehci_class_init,
    .class_size    = sizeof(SRISAEHCIClass),
};
...
static void ehci_sysbus_register_types(void)
{
    ...
    type_register_static(&ehci_srisa_sc64_type_info);
}
