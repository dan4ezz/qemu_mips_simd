...
struct MemoryRegionOps {
    ...
    /* If present, checks if operation is permitted and if not the operation
     * is aborted returning error. This is due to all memory_region_*_accessor
     * always return MEMTX_OK, and if `accepts` callback fails it calls
     * unassigned handler which may cause (or may not - depends on the context)
     * an exception in guest system.
     */
    MemTxResult (*check_access)(void *opaque,
                               hwaddr addr,
                               int len,
                               bool is_write);
