...
static bool access_denied(MemoryRegion *mr, hwaddr addr, int len, bool is_write)
{
        if (mr->ops && mr->ops->check_access) {
            MemTxResult rc;
            rc = mr->ops->check_access(mr->opaque, addr, len, true);
            return rc != MEMTX_OK;
        }
        return false;
}
                                           ...
                                           hwaddr l, MemoryRegion *mr)
{
    ...
    for (;;) {
        if (!memory_access_is_direct(mr, true)) {
            ...
            if (access_denied(mr, addr1, l, true))
                return MEMTX_ERROR;
                                   ...
                                   MemoryRegion *mr)
{
    ...
    for (;;) {
        if (!memory_access_is_direct(mr, false)) {
            ...
            if (access_denied(mr, addr1, l, false))
                return MEMTX_ERROR;
